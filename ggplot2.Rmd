...
![](https://oestatistico.com.br/wp-content/uploads/2016/08/Grammar-graphhics-1.png)
...
```{r}
dataset %>% ggplot(aes(x = total_passgrs, y = total_fatal_injuries)) + geom_point()
```

```{r}
dataset %>% ggplot(aes(x = total_passgrs, y = total_fatal_injuries)) + geom_density() +
    scale_colour_viridis() + facet_grid(~aircraft_category)
```
