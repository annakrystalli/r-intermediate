...
date: "adapted from content by Thomas Carroll"
output:
  html_document:
    df_print: paged
    toc: yes
...
> **Data Visualisation:** The _representation_ and _presentation_ of _data_ to _facilitate understanding_.

---

## data

### **data is:** _names, amounts, groups, statistical values, dates, comments locations..._




<figure>
<img src="images/data.svg" width="400">
<figcaption>source: <a href="https://docs.google.com/presentation/d/10L-yA_Tq2-3r70F8EZHE0y5GCEE4214oJHucUclOhX8/edit#slide=id.g140a4aa7b2_1_0">Vega-Lite tutorial slides</a> by Dominik Moritz & Kanit “Ham” Wongsuphasawat
</figcaption>
</figure>
...
We need a better representation to extract meaning
...
### start with data: `mtcars`

```{r, echo=FALSE}
mtcars
```

### encode a variable on an axis
<img width="400" src="images/pl1.svg">

### add another variable using another axis
<img width="400" src="images/pl3.svg">

### encode a thrid variable using colour
<img width="400" src="images/pl4.svg">
...
<img src="images/gog.jpg" height="100px" width="100px"/>
...
[**RStudio**](https://www.rstudio.com/) provides an excellent interactive environment to analyse and visualise data.
_ensure you have version > 1.0 installed. Download latest version [here](https://www.rstudio.com/products/rstudio/#Desktop)._
...
<img src="images/lgg.png" height="100px" width="150px"/>
<img src="images/ggp_book.jpg" height="100px" width="150px"/>
...
```{r load_packages, eval=TRUE,warning=F,message=F}
library(tidyr)
library(ggplot2)
library(dplyr)
library(stringr)
...
[![](http://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.png)](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf)
...
We can access elements of the plot object using `$`.
...
A caution on using barplots:

<img src="https://pagepiccinini.files.wordpress.com/2016/02/barplot_psa1.jpg" height="300" width="320">
<img src="https://ksr-ugc.imgix.net/assets/011/118/050/2a0e6672e417ed2a12fac1b1828d309a_original.png?w=680&fit=max&v=1462543835&auto=format&lossless=true&s=28a174592b91b6b590e0b35da3ade2a0" height="300" width="600">

Good for counts and representing absolute differences in counts. 
...
## pirateplot - `yarrr` `r emo::ji("package")`
...
```{r}
patients_clean %>% pirateplot(formula = Weight ~ Sex,
           data = .,
           theme = 2)
```
...
An overview of geoms and their arguments can be found at ggplot2 documentation or within the ggplot2 cheatsheet.
...
pcPlot <- ggplot(patients_clean, aes(x=Height, y=Weight, colour=Sex))
pcPlot + geom_point()
```

```{r, scatter_aes_sexColourFill}
pcPlot <- ggplot(patients_clean, 
                 aes(x=Height, y = Weight, 
                     fill = Sex))
pcPlot + geom_point(colour = "white", shape = 21, size = 3)
```
...
```{r, aes_in_geom}
pcPlot <- ggplot(data=patients_clean, aes(x=Height,y=Weight,colour=Sex, shape=Smokes)) + geom_point()
```
...
We can map additional attributes to bars also. The default is to stack colours to indicate the distribution of counts of a discrete variable across another discrete variable. 

```{r}
pcPlot <- ggplot(data=patients_clean,
        mapping=aes(x=Sex, fill=Pet))

pcPlot + geom_bar() 
```
It's hard however to compare the distribution of pets accurately. We can present the counts as proportions using `position_fill()`
```{r}
pcPlot + geom_bar(position=position_fill()) 
```
Or to give them a common base, we can instead **dodge** the bars

```{r}
pcPlot + geom_bar(position="dodge2") 
```


```{r}
pcPlot <- ggplot(data=patients_clean,
        mapping=aes(x=Pet, fill=Sex))
...
<figure>
<img src="images/data_types.svg" width="400">
<img src="images/visual-encoding.svg" width="400">
...
## ggplot has good defaults



### categorical data trigger `scale_colour_hue()` or `scale_fill_hue()`


```{r, echo = F, fig.height=1.3}
library(colorspace)
...
n <- 5
pal <- gg_color_hue(n) %>% setNames(letters[1:n])
df <- data.frame(x=letters[1:n])
ggplot(data=df, aes(x), fill=x) + 
    geom_bar(fill = pal) + 
...
### continuous data trigger `scale_colour_gradient()` or `scale_fill_gradient()`
...
### [Colorbrewer](http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) for discrete colour scale
...
Palletes available through package [`RColorBrewer`](https://cran.r-project.org/web/packages/RColorBrewer/RColorBrewer.pdf)

```{r}
RColorBrewer::display.brewer.all(n=20, exact.n=FALSE) 
```
...
Shape is mainly used for categorical data and ggplot maps a maximum of the following 6 shapes to categories by default

```{r, echo=F, fig.height=1.3}
d=data.frame(a=letters[1:6])
ggplot(data = d) +
...
For more categories or to use any of the other shapes available through R, we need to set the shapes manually.
...
pcPlot+stat_summary(fun.y=quantile, geom="point",
                    shape = "-", colour= pal, size=8)
```



## pirate plot in ggplot2

Let's reproduce a pirate plot with ggplot

Let's start with the raw data. One axis is categorical, so just plotting the data with `geom_point` just plots a string of data points. 

```{r}
p <- patients_clean %>% ggplot(aes(x = Sex, y = Weight, color = Sex, fill = Sex)) 
p + geom_point()
    
```

So we can try and **jitter** the points, adding random noise to their coordinates to try and disperse the points along the x axis. This is particularly useful for small datasets with at least one discrete position.
...
Another approach when faced with too many data points is to use a `dotplot` instead.

```{r}
p + geom_dotplot(binaxis='y', stackdir='center', dotsize=1)
```
Now we can add the violin plot.
...
Now we can layer representations of statistical summaries.

For example, we can add a visua representation of the distribution of `Weight` using `geom_boxplot`.

```{r}
p + geom_dotplot(binaxis='y', stackdir='center', dotsize=1) +
    geom_violin(fill = NA) +
    geom_boxplot(width=0.1, fill = "white", alpha = 0.60)
```

```{r}
p + geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5) +
    geom_violin(fill = NA) +
    #stat_summary(fun.y = mean, geom = "crossbar", fill = NA, colour = "black", fatten = 4) + 
    stat_summary(fun.data = mean_se, geom = "crossbar", fatten = 2, width = 0.5, 
...
p + geom_dotplot(binaxis='y', stackdir='center', dotsize=0.5) +
    geom_violin(fill = NA) +
    #stat_summary(fun.y = mean, geom = "crossbar", fill = NA, colour = "black", fatten = 4) + 
    stat_summary(fun.data = mean_se, geom = "bar", fatten = 2, width = 0.5, 
                 alpha = 0.5, color = NA)
```
...
```{r}
pcPlot <- ggplot(patients_clean, aes(x=Height, y=Weight, colour=Sex, shape = Smokes, linetype = Smokes))
pcPlot + geom_point() + geom_smooth(method = "lm")
```
